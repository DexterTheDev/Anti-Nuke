...
module.exports = async (client, channel) => {
    ...
    if (limiter.get(executor.id) >= 4) {
        let member = await channel.guild.members.cache.get(executor.id);
        let roles = [];
        await member.roles.cache.map(r => {
            if (r.id != channel.guild.id) roles.push(r.name);
        });
        ...
        client.logger(content = "**⚠️ Something has been detected**", embed = new MessageEmbed()
            ...
            .setColor("RED")
            .setDescription(`<@${executor.id || "Invalid"}> (**${executor.tag || "Invalid#0000"}**) has triggered \`WEBHOOKS DELETION/CREATION\` anti nuke.`)
